...
第二步，设置一台mysql服务器，可以是自己虚拟机上面的MySQL，mysql创建一个数据库，我的是buoluo,然后将mysql的连接信息在config.json修改
第三步，运行dockerfile就可以进行连接查看。
```
下面直接dockerfile,假如没有fortify，只是用里面渗透测试系统，可以直接注释掉那段fortify的配置。在buoluo目录下有一个config.json文件，里面有所有的配置，在生成docker的时候，把config.json复制出来，修改里面的内容。
```
#dockerfile
...
#config.json
COPY config.json /opt/buoluo/buoluo/
...
#run.sh
redis-server & 
cd /opt/buoluo && python3 manage.py celery -A buoluo worker  -l info --beat & 
cd /opt/buoluo/ &&  python3 manage.py runserver 0.0.0.0:8000
...
```angular2
[root@1c4e68368d53 buoluo]# touch /opt/git-list
[root@1c4e68368d53 buoluo]# vim /opt/git-list
[root@1c4e68368d53 buoluo]# cat /opt/git-list 
